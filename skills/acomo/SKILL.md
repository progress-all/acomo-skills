...
```bash
acomo listWorkflowModels -f summary
```

モデル ID、名前、ノード数、フィールド数の簡潔な一覧を取得します。
...
- **`listWorkflowModels -f summary`**: 利用可能なワークフローモデル一覧（ID・名前・ノード数・フィールド数）を簡潔に取得。どのモデルを扱うか選ぶときに使う。
